...
# --- CONFIGURATION (Updated for gardener project) ---
...
FILES_TO_SUMMARIZE = {
    ...
    "core/services/ingestion.py": "views_and_forms",
    "core/services/processing.py": "views_and_forms",
    "core/services/enrichment.py": "views_and_forms",
    "core/tests.py": "tests",
}
OUTPUT_DIR = PROJECT_ROOT / "product" / "context"
OUTPUT_FILE = OUTPUT_DIR / "CURRENT_STATE.md"
IGNORE_DIRS = {"__pycache__", ".venv", "venv", ".git", "migrations", "static", "media", ".pytest_cache"}
...
def summarize_views_and_forms(file_path):
    ...
    for node in ast.walk(tree):
        ...
        final_signature = f"{'class' if isinstance(node, ast.ClassDef) else 'def'} {signature_base}:"
        if comments:
...
print(f"Summarized context file '{OUTPUT_FILE}' generated successfully.")